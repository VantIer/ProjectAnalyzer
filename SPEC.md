...
| 步骤 | 内容 | 输出 |
|------|------|------|
| 2 | 检测文件变化 | 对比快照，识别新增/修改/删除文件，清理已删除文件的 `ana_*.md` |
...
- 增量模式：加载快照对比当前文件哈希，识别新增/修改/删除，清理已删除文件对应的 `ana_*.md`
  - 无快照时：自动执行全量分析
  - 仅预览变化不执行分析：使用 `--diff --step 2`
...
12. `--diff-init` 仅生成快照不执行分析
13. `--diff --step 2` 可仅预览变化不执行分析
14. 详细日志保存到 `{项目目录名}.log`
